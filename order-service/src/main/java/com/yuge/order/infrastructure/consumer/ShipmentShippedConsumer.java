...
import com.yuge.order.domain.event.ShipmentShippedEvent;
 ...
 * 发货事件消费者
 * 
 * 消费 ShipmentShipped 事件，推进订单状态：PAID -> SHIPPED
 */
@Slf4j
@Component
...
@RocketMQMessageListener(
        ...
        selectorExpression = "SHIPMENT_SHIPPED",
        consumerGroup = "order-shipment-shipped-consumer-group"
)
public class ShipmentShippedConsumer implements RocketMQListener<MessageExt> {

    private static final String CONSUMER_GROUP = "order-shipment-shipped-consumer-group";
    ...
    public void onMessage(MessageExt messageExt) {
        ...
        if (baseEvent == null) {
            log.error("[ShipmentShippedConsumer] Failed to parse message, msgId={}", msgId);
            return;
        }
        ...
        try {
            log.info("[ShipmentShippedConsumer] Received message, eventId={}, bizKey={}, msgId={}, reconsumeTimes={}",
            ...
            if (existingRecord.isPresent()) {
                ...
                if (ConsumeStatus.SUCCESS.getCode().equals(record.getStatus()) ||
                    ...
                    log.info("[ShipmentShippedConsumer] Message already processed, eventId={}, status={}",
                            eventId, record.getStatus());
                    return;
                } else if (ConsumeStatus.PROCESSING.getCode().equals(record.getStatus())) {
                    log.info("[ShipmentShippedConsumer] Message is being processed, eventId={}", eventId);
                    throw new RuntimeException("Message is being processed by another instance");
                }
            }

            // 4. 解析发货事件
            ShipmentShippedEvent shippedEvent = parseShipmentShippedEvent(baseEvent.getPayload());
            if (shippedEvent == null) {
                log.error("[ShipmentShippedConsumer] Failed to parse ShipmentShippedEvent, eventId={}", eventId);
            ...
            // 5. 处理发货事件
            processShipmentShipped(shippedEvent, eventId, startTime);

        } catch (Exception e) {
            long costMs = System.currentTimeMillis() - startTime;
            log.error("[ShipmentShippedConsumer] Failed to consume message, eventId={}, error={}",
            ...
            if (reconsumeTimes < 3) {
                ...
                log.error("[ShipmentShippedConsumer] Max retry reached, eventId={}", eventId);
            }
        } finally {
            TraceContext.clear();
    ...
    public void processShipmentShipped(ShipmentShippedEvent event, String eventId, long startTime) {
        String orderNo = event.getOrderNo();
        String shipmentNo = event.getShipmentNo();
        String waybillNo = event.getWaybillNo();

        log.info("[ShipmentShippedConsumer] Processing ShipmentShipped, orderNo={}, shipmentNo={}, waybillNo={}, eventId={}",
                orderNo, shipmentNo, waybillNo, eventId);

        // 1. 查询订单
        Optional<Order> orderOpt = orderRepository.findByOrderNo(orderNo);
        if (orderOpt.isEmpty()) {
            log.error("[ShipmentShippedConsumer] Order not found, orderNo={}", orderNo);
        ...
        // 2. 幂等检查：如果已经是SHIPPED或DELIVERED状态，直接返回成功
        if (currentStatus == OrderStatus.SHIPPED || currentStatus == OrderStatus.DELIVERED) {
            log.info("[ShipmentShippedConsumer] Order already shipped/delivered, orderNo={}, status={}",
                    orderNo, currentStatus);
            mqConsumeLogRepository.markIgnored(eventId, CONSUMER_GROUP,
                    "订单已发货/签收", "订单已处于" + currentStatus.getDesc() + "状态",
                    System.currentTimeMillis() - startTime);
            return;
        }

        // 3. 检查状态机是否允许转换
        if (!orderStateMachine.canTransition(currentStatus, OrderEvent.SHIP)) {
            String ignoredReason = String.format("当前状态[%s]不允许执行[%s]事件，可能是乱序消息",
                    currentStatus.getDesc(), OrderEvent.SHIP.getDesc());
            
            log.warn("[ShipmentShippedConsumer] Out-of-order message detected, orderNo={}, currentStatus={}, eventId={}",
                    orderNo, currentStatus, eventId);

            orderRepository.saveIgnoredStateFlow(orderNo, currentStatus.getCode(),
                    OrderEvent.SHIP, eventId, ignoredReason);
        ...
        // 4. CAS 更新订单状态：PAID -> SHIPPED
        boolean updated = orderRepository.casUpdateStatusOnly(orderNo, OrderStatus.PAID, OrderStatus.SHIPPED);

        if (updated) {
            orderRepository.saveStateFlow(orderNo, OrderStatus.PAID, OrderStatus.SHIPPED,
                    OrderEvent.SHIP, eventId, "SYSTEM",
                    String.format("发货成功，发货单号: %s, 运单号: %s", shipmentNo, waybillNo));

            mqConsumeLogRepository.markSuccess(eventId, CONSUMER_GROUP,
                    "状态更新成功: PAID -> SHIPPED",
                    System.currentTimeMillis() - startTime);

            log.info("[ShipmentShippedConsumer] Order status updated, orderNo={}, PAID -> SHIPPED, shipmentNo={}",
                    orderNo, shipmentNo);
        } else {
            ...
            if (OrderStatus.SHIPPED.getCode().equals(newStatus) ||
                OrderStatus.DELIVERED.getCode().equals(newStatus)) {
                mqConsumeLogRepository.markIgnored(eventId, CONSUMER_GROUP,
                        "订单已发货/签收", "CAS更新失败，订单已处于" + newStatus + "状态",
                        System.currentTimeMillis() - startTime);
                log.info("[ShipmentShippedConsumer] Order already shipped/delivered (concurrent), orderNo={}", orderNo);
                return;
            }

            String ignoredReason = String.format("CAS更新失败，订单当前状态为[%s]，期望状态为[PAID]，可能是并发或乱序",
                    newStatus);

            log.warn("[ShipmentShippedConsumer] CAS update failed (out-of-order), orderNo={}, currentStatus={}, eventId={}",
    ...
    private BaseEvent parseBaseEvent(MessageExt messageExt) {
        try {
            ...
            log.error("[ShipmentShippedConsumer] Failed to parse BaseEvent, msgId={}",
                    messageExt.getMsgId(), e);
            return null;
        }
    }

    private ShipmentShippedEvent parseShipmentShippedEvent(String payload) {
        try {
            return objectMapper.readValue(payload, ShipmentShippedEvent.class);
        } catch (Exception e) {
            log.error("[ShipmentShippedConsumer] Failed to parse ShipmentShippedEvent payload", e);
            return null;
        }
    }
}
